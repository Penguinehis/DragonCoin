#!/usr/bin/env python3
"""
DragonCoin Official PC Miner 4.2 © MIT licensed
https://duinocoin.com
https://github.com/PenguinEhis/DragonCoin
DragonCoin Team & Community 2019-2024
"""
...
def handler(signal_received, frame):
    ...
    if sys.platform == "win32":
        ...
        Popen(
            "kill $(ps aux | grep PC_Miner | awk '{print $2}')", shell=True, stdout=PIPE
        )
...
class Settings:
    ...
    SEPARATOR = ","
    VER = 4.2
    DATA_DIR = "DragonCoin PC Miner " + str(VER)
    TRANSLATIONS = (
...
def check_updates():
    ...
    try:
        data = requests.get(
            "https://api.github.com/repos/PenguinEhis/DragonCoin/releases/latest"
        ).json()

        zip_file = "DragonCoin_" + data["tag_name"] + "_linux.zip"
        if sys.platform == "win32":
            zip_file = "DragonCoin_" + data["tag_name"] + "_windows.zip"
        ...
        if float(Settings.VER) < float(data["tag_name"]):  # If is outdated
            ...
            if update.lower() == "y" or update == "":
                ...
                DATA_DIR = "DragonCoin PC Miner " + str(
                    data["tag_name"]
                )  # Create new version config folder
                ...
                try:
                    ...
                    with open(
                        str(DATA_DIR)  # save it on the new version folder
                        + "/Settings.cfg",
                        "w",
                    ) as configfile:
                    ...
                    pretty_print(
                        Style.RESET_ALL + get_string("config_saved"), "success", "sys0"
                    )
                except Exception as e:
                    pretty_print(
                        f"Error saving configfile: {e}" + str(e), "error", "sys0"
                    )
                    pretty_print(
                        "Config won't be carried to the next version", "warning", "sys0"
                    )
                ...
                download_url = (
                    "https://github.com/PenguinEhis/DragonCoin/releases/download/"
                    + data["tag_name"]
                    + "/"
                    + zip_file
                )
                ...
                if running_script:
                    file_path = os.path.join(
                        ".", "PC_Miner_" + data["tag_name"] + ".py"
                    )
                    download_url = "https://raw.githubusercontent.com/PenguinEhis/DragonCoin/master/PC_Miner.py"
                ...
                if r.ok:
                    ...
                    pretty_print(
                        f"Saving update to: {os.path.abspath(file_path)}",
                        "warning",
                        "sys0",
                    )
                    ...
                    if not running_script:
                        ...
                        with zipfile.ZipFile(
                            file_path, "r"
                        ) as zip_ref:  # Unzip the file
                            for file in zip_ref.infolist():
                                if "PC_Miner" in file.filename:
                                    if sys.platform == "win32":
                                        file.filename = (
                                            "PC_Miner_" + data["tag_name"] + ".exe"
                                        )  # Rename the file
                                    else:
                        ...
                        if sys.platform == "win32":
                            os.startfile(
                                os.getcwd() + "\\PC_Miner_" + data["tag_name"] + ".exe"
                            )  # Start the miner
                    ...
                    pretty_print(
                        f"Update failed: {r.status_code}: {r.text}", "error", "sys0"
                    )
            else:
...
class Algorithms:
    ...
    https://github.com/PenguinEhis/DragonCoin/blob/gh-pages/assets/whitepaper.pdf
    """
...
def get_rpi_temperature():
    output = (
        Popen(args="cat /sys/class/thermal/thermal_zone0/temp", stdout=PIPE, shell=True)
        .communicate()[0]
        .decode()
    )
...
def pretty_print(
    msg: str = None, state: str = "success", sender: str = "sys0", print_queue=None
):
    """
    ...
    def preload():
        ...
        if not Path(Settings.DATA_DIR + Settings.TRANSLATIONS_FILE).is_file():
            with open(Settings.DATA_DIR + Settings.TRANSLATIONS_FILE, "wb") as f:
                f.write(
                    requests.get(
                        Settings.TRANSLATIONS, timeout=Settings.SOC_TIMEOUT
                    ).content
                )

        with open(
            Settings.DATA_DIR + Settings.TRANSLATIONS_FILE,
            "r",
    ...
    def load_cfg():
        ...
        if not Path(Settings.DATA_DIR + Settings.SETTINGS_FILE).is_file():
            ...
            start_diff = sub(
                r"\D",
                "",
                input(Style.NORMAL + get_string("ask_difficulty") + Style.BRIGHT),
            )
            if start_diff == "1":
                start_diff = "LOW"
            elif start_diff == "3":
            ...
            rig_id = input(
                Style.NORMAL + get_string("ask_rig_identifier") + Style.BRIGHT
            )
            if rig_id.lower() == "y":
                rig_id = str(
                    input(Style.NORMAL + get_string("ask_rig_name") + Style.BRIGHT)
                )
            else:
                rig_id = "None"
                ...
                while True:
                    try:
                        ...
                        if user_settings["raspi_cpu_iot"] == "y" and running_on_rpi:
                            ...
                            raspi_iot_reading = (
                                f"CPU temperature:{get_rpi_temperature()}*C"
                            )
                        ...
                        pretty_print(
                            get_string("error_while_mining") + " " + str(e),
                            "error",
                            "net" + str(id),
                            print_queue=print_queue,
                        )
            ...
            if int(python_version_tuple()[1]) <= 6:
                ...
                pretty_print(
                    (
                        ...
                        + "https://github.com/PenguinEhis/DragonCoin/wiki/"
    ...
    def load():
        if os.name == "nt":
            if not Path("libducohasher.pyd").is_file():
                ...
                url = (
                    "https://host.dragoncoressh.com/" + "fasthash/libducohashWindows.pyd"
                )
            ...
            if osprocessor() == "aarch64":
                url = "https://host.dragoncoressh.com/" + "fasthash/libducohashPi4.so"
            elif osprocessor() == "armv7l":
                url = "https://host.dragoncoressh.com/" + "fasthash/libducohashPi4_32.so"
            elif osprocessor() == "armv6l":
                url = "https://host.dragoncoressh.com/" + "fasthash/libducohashPiZero.so"
            elif osprocessor() == "x86_64":
                url = "https://host.dragoncoressh.com/" + "fasthash/libducohashLinux.so"
            else:
                pretty_print(
                    (
                        "Fasthash accelerations are not available for your OS.\n"
